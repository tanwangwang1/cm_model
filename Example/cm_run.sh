...
savefile="/home/matteo/github_2/clustering_module/Example/experiments_d1023_mnist/alpha_1.04_new_alpha_${new_alpha}_temp_${temperature}.txt"
python3 cm_mnist.py  -n $new_alpha > $savefile

# while [ $(echo "$temperature< 60" | bc) -eq 1 ]; do
#   new_alpha=0.10
#   while [ $(echo "$new_alpha< 0.4" | bc) -eq 1 ]; do
#       # 调用 Python 脚本
#       savefile="/home/matteo/github_2/clustering_module/Example/experiments_d1022_mnist/alpha_1.04_new_alpha_${new_alpha}_temp_${temperature}.txt"
#       python3 cm_mnist.py  -n $new_alpha > $savefile
#       # 增加 t 参数值
