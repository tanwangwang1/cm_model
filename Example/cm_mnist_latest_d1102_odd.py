...
save_folder = "/home/linux/clustering_module/experiments_d1104/mnist_odd"
BETA = 200.
def parse_arguments():
    """
    ...
    parser.add_argument("--new_alpha",'-n',type=float, default=10**-3, help='set the added alpha [0.1,1], rename as new_alpha')
...
def evaluate_umap(
    ...
    if is_save:
        ...
        plt.legend(loc='best')
        savefig = f"{save_folder}/umap_final_alpha_{alpha}_new_alpha_{new_alpha}_temp_{temp}.png"
...
def evaluate(
    ...
    if is_save:
        ...
        savefig = f"{save_folder}/umap_final_alpha_{alpha}_new_alpha_{new_alpha}_temp_{temp}_all.png"
        plt.savefig(savefig, dpi=300)
        plt.close()
...
def plot_loss_components(cm_loss_list, alpha, new_alpha, temp):
    ...
    # 绘制reconstruction loss图
    plt.figure(figsize=(8, 6))
    plt.plot(reconstruction_losses, color='blue')
    plt.xscale('log')
    plt.yscale('log')
    ...
    plt.tight_layout()
    plt.savefig(f"{save_folder}/reconstruction_loss_alpha_{alpha}_new_alpha_{new_alpha}_temp_{temp}.png")
    plt.close()

    # 创建 2x2 子图，用于显示 clustering loss 的不同视图
    fig, axs = plt.subplots(2, 2, figsize=(16, 8))
    views = ['View 1', 'View 2', 'View 3', 'View 4']
    colors = ['blue', 'orange', 'green', 'red']

    for i, ax in enumerate(axs.flatten()):
        ax.plot(clustering_losses, color=colors[i])
        ax.set_xscale('log')
        ax.set_yscale('log')
        ax.set_xlabel('Iteration(log scale)')
        ax.set_ylabel('Loss(log scale)')
        ax.set_title(f'Loss {i + 1}')
    fig.suptitle('Clustering Loss', fontsize=16)
    plt.tight_layout()
    plt.savefig(f"{save_folder}/clustering_loss_views_alpha_{alpha}_new_alpha_{new_alpha}_temp_{temp}.png")
    plt.close()
...
#     # 调整布局
#     plt.tight_layout()
#     savepath = f"{save_folder}/alpha_{alpha}_new_alpha_{new_alpha}_temp_{temp}.png"
...
#     # 调整布局
#     plt.tight_layout()
#     savepath =f"{save_folder}/alpha_{alpha}_new_alpha_{new_alpha}_temp_{temp}_4parts.png" 
#     plt.savefig(savepath)
def avg_epoch(model,
              dataloader,
              criterion_cluster,
...
def main(args):
    ...
    def filter_by_labels(dataset):
        allowed_labels = [1, 3, 5, 7, 9]
    ...
    train_dataset = filter_by_labels(train_dataset)
    test_dataset = filter_by_labels(test_dataset)
