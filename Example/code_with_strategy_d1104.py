...
def evaluate(model,dataloader,criterion_cluster,optimizer,device, epoch,
             ...
             temp=None, seed_=None,cm_loss_list=None):
    model.eval()
    pred,lbl,x = [],[],None
    X_all = []
    y_all = []
    ...
    if is_save:
        ...
        # 定义损失数组、标题、颜色和y轴范围
        losses = [
            (cm_loss_1, 'Loss 1', 'blue', (0, 1.1)),
            (cm_loss_2, 'Loss 2', 'orange', (0, 0.4)),
            (cm_loss_3, 'Loss 3', 'green', (-0.4, 0.02)),
            (cm_loss_4, 'Loss 4', 'red', (0.8, 1.4))
        ]
        ...
        # 遍历每个子图并绘制对应的损失曲线
        for ax, (loss, title, color, (y_min, y_max)) in zip(axs, losses):
            ax.plot(epochs, loss, color=color, linestyle='-')
            ax.set_title(title, fontsize=14)
            ax.set_xlabel('Iterations', fontsize=18)
            ax.set_ylabel('Losses', fontsize=18)
            ax.set_ylim([y_min, y_max])  # 为每个子图设置不同的y轴区间
            #ax.grid(True)
            ax.tick_params(axis='both', which='major', labelsize=14)  # 设置主刻度标签字体大小
            ax.tick_params(axis='both', which='minor', labelsize=14)  # 设置次刻度标签字体大小
        ...
        csv_filename = f'/home/matteo/github_2/experiments_d1104/with_strategy/3blobs5centroids/seed_{seed_}/all_scores_3blobs_5centroids.csv'
        ...
        plot_predictions(pred[:],X_all,model._mu().detach().cpu().numpy(), blob_fig, alpha,c_alpha, centroids,temp,acc, 
