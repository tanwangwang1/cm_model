...
def parse_arguments():
    ...
    parser.add_argument("--c_alpha",'-ca',type=float, default=0.2, required=True, help='set the amount of centroids')
    parser.add_argument("--freq",'-f', type=int, default=1,help='the savepath of plots')
    parser.add_argument("--seed",'-s', type=int, default=31,help='the savepath of plots')
...
def evaluate(model,dataloader,criterion_cluster,optimizer,device, epoch,savepath,full=False,is_save=False, alpha=None, centroids=None, c_alpha=None):
    model.eval()
    pred,lbl,x = [],[],None
    X_all = []
    ...
    import time
    save_path = savepath + f"/c{centroids}a{c_alpha}.png"
    if is_save:
        ...
        if (epoch)%1 == 0:
            ...
            if (epoch+1)%10 == 0: 
                with torch.no_grad(): 
                    ...
                    criterion_cluster.alpha = criterion_cluster.alpha*args.c_alpha + (torch.tensor(freq+args.freq).float()*(1 - args.c_alpha)).to(device)
