    ...
    if is_save:
        ...
        if acc >= 1:
            ...
            plot_predictions(pred[:],X_all,model._mu().detach().cpu().numpy(), save_path, alpha,c_alpha, centroids,temp,acc, 
                             ...
                             normalized_score, homogeneity_score, completeness_score, v_measure_score,cm_loss)
...
def plot_predictions(y_pred, X, C, save_path,alpha,c_alpha,centroids,temp,acc, 
    ...
    plt.text(0.5, -0.1, f'Acc:{acc}  Silhouette:{silhouette_score_}  Davies:{davies_index} Adjusted:{adjusted_score} \n Normalized:{normalized_score} Homogeneity:{homogeneity_score} Completeness: {completeness_score}V_measure:{v_measure_score_} std(E1): {} std(E2): {} std(E3): {} std(E4): {}', ha='center', va='center', transform=plt.gca().transAxes)
    if save_path:
