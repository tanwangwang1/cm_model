...
def evaluate(model,dataloader,criterion_cluster,optimizer,device, epoch,savepath=None,full=False,is_save=False, alpha=None, centroids=None, c_alpha=None, temp=None):
    model.eval()
    pred,lbl,x = [],[],None
    X_all = []
    ...
    if is_save:
        ...
        SS = round(silhouette_score(X_all, pred), 3)
        CHI = round(calinski_harabasz_score(X_all, pred), 3)
        DBI = round(davies_bouldin_score(X_all, pred), 3)
        import pandas as pd
        csv_filename = '/home/matteo/thesis_2/clustering_module/Example/d0711.csv'
        if SS >= 0.599:
            ...
            plot_predictions(pred[:],X_all,model._mu().detach().cpu().numpy(), save_path, alpha,c_alpha, centroids,temp,SS, CHI, DBI)
...
def plot_predictions(y_pred, X, C, save_path,alpha,c_alpha,centroids,temp,SS,CHI,DBI):
    colors = np.array(
        list(
            islice(
                cycle(
    ...
    plt.text(0.5, -0.1, f'SS:{SS}  CHI:{CHI}   DBI:{DBI}', ha='center', va='center', transform=plt.gca().transAxes)
    # plt.text(
    #     0.99,
    #     0.01,
