...
class Clustering_Module(nn.Module):
    ...
    def __init__(self, input_dim, num_clusters, use_bias=False):
        ...
        self.gamma = nn.Sequential( #根据输入数据计算聚类分配（软分配）
            nn.Linear(input_dim, num_clusters, bias=use_bias),
            nn.Softmax()
        )
        self.mu = nn.Sequential( #根据聚类分配计算聚类中心。
            nn.Linear(num_clusters, input_dim, bias=use_bias),
        )

    def _mu(self):  #根据单位矩阵计算聚类中心
        return self.mu( self.Id )
        
    def predict(self, x):#为输入数据预测硬聚类分配
        return self.gamma(x).argmax(-1)
        
    def predict_proba(self, x): #输入数据预测软聚类分配
        return self.gamma(x)
        
    def forward(self, x):   #通过聚类模块进行前向传播。它计算软聚类分配、聚类中心，并返回各种中间结果。
        g  = self.gamma(x)
        tx = self.mu(g)
